        ...
        new Thread() {
            ...
            public void run() {
                ...
                if ((error = mReader.powerOn()) != IDCardReader.RESULT_OK) {
                    showErrorDialog(getString(R.string.id_card_reader_power_on_failed), getIDCardErrorString(error));
                }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if ((error = mReader.powerOff()) != IDCardReader.RESULT_OK) {
                    showErrorDialog(getString(R.string.id_card_reader_power_off_failed), getIDCardErrorString(error));
                }
                if (finish) {
                    finishActivity();
                }
            ...
            if (params[0].equals("read")) {
                ...
                if (!isCancelled()) {
                    if (res.error == IDCardReader.RESULT_OK) {
                        ...
                        showInfoToast(getString(R.string.id_card_read_id_success) + sb.toString());
                    } else {
                        showErrorDialog(getString(R.string.id_card_read_id_failed), getIDCardErrorString(res.error));
                    }
                }
                dismissProgressDialog();
            }
