    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SHOW_INFO: {
                    Toast.makeText(ICCardDemo.this, (String) msg.obj, Toast.LENGTH_SHORT).show();
                    break;
                }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if ((error = mReader.powerOn()) != ICCardReader.RESULT_OK) {
                    showErrorDialog(getString(R.string.ic_card_reader_power_on_failed), getICCardErrorString(error));
                }
                if ((error = mReader.open()) != ICCardReader.RESULT_OK) {
                    showErrorDialog(getString(R.string.ic_card_reader_open_failed), getICCardErrorString(error));
                } else {
                    enableControl(true);
                    showInfoToast(getString(R.string.ic_card_reader_open_success));
                }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if ((error = mReader.close()) != ICCardReader.RESULT_OK) {
                    showErrorDialog(getString(R.string.ic_card_reader_close_failed), getICCardErrorString(error));
                } else {
                    showInfoToast(getString(R.string.ic_card_reader_close_success));
                }
                if ((error = mReader.powerOff()) != ICCardReader.RESULT_OK) {
                    showErrorDialog(getString(R.string.ic_card_reader_power_off_failed), getICCardErrorString(error));
                }
                if (finish) {
                    finishActivity();
                }
            ...
            if (params[0].equals("read")) {
                ...
                if (!isCancelled()) {
                    if (res.error == ICCardReader.RESULT_OK) {
                        ...
                        showInfoToast(getString(R.string.ic_card_read_success));
                        showCardInfo(sb.toString());
                    } else {
                        showErrorDialog(getString(R.string.ic_card_read_failed), getICCardErrorString(res.error));
                    }
                }
                dismissProgressDialog();
                ...
                if (!isCancelled()) {
                    if (res.error == ICCardReader.RESULT_OK) {
                        ...
                        if (error == ICCardReader.RESULT_OK) {
                            ...
                            if (error == ICCardReader.RESULT_OK) {
                                showInfoToast(getString(R.string.ic_card_write_success));
                            } else {
                                showErrorDialog(getString(R.string.ic_card_write_failed), getICCardErrorString(res.error));
                            }
                        } else if (error == ICCardReader.NOT_SUPPORT) {
                            // showErrorDialog(getString(R.string.ic_card_not_support), null);
                        } else {
                            showErrorDialog(getString(R.string.ic_card_validate_key_failed), null);
                        }
                    } else {
                        showErrorDialog(getString(R.string.ic_card_write_failed), getICCardErrorString(res.error));
                    }
                    ...
                    if (res.error == ICCardReader.RESULT_OK) {
                        ...
                        if (error == ICCardReader.RESULT_OK) {
                            ...
                            if (error == ICCardReader.RESULT_OK) {
                                ...
                                showErrorDialog(getString(R.string.ic_card_write_failed), getICCardErrorString(error));
                            }
                        } else if (error == ICCardReader.NOT_SUPPORT) {
                            // showErrorDialog(getString(R.string.ic_card_not_support), null);
                        } else {
